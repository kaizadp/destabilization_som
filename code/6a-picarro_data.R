...
compute_ghg_fluxes <- function(p_clean_matched, valve_key) {
  ...
  p_clean_matched %>% 
    ...
    ungroup() %>% 
    left_join(valve_key %>% dplyr::select(Core, goethite, treatment_mgC))
}
...
qc_fluxes <- function(ghg_fluxes, valve_key) {
  ...
  p_co2 <- ggplot(ghg_fluxes, aes(DATETIME, flux_co2_umol_g_s, group = Core, color = Core)) + 
    geom_point() + geom_line() +
    facet_grid(. ~ goethite)
  ggsave("outputs/fluxes_co2.pdf", plot = p_co2, width = 8, height = 6)
  p_ch4 <- ggplot(ghg_fluxes, aes(DATETIME, flux_ch4_nmol_g_s, group = Core, color = Core)) + 
    geom_point() + geom_line() +
    facet_grid(. ~ goethite)
...
filter_positive_slope <-function(p_clean_matched){
  p_clean_matched %>% 
    mutate(rownumber = row_number()) %>%
    group_by(Core, Sample_number) %>% 
    mutate(row = case_when(CO2_dry == max(CO2_dry) ~ rownumber),
           row2 = max(row, na.rm = TRUE),
           keep = row2 >= rownumber) %>% 
    filter(keep) %>% 
    dplyr::select(-row, -row2, -keep)
}
